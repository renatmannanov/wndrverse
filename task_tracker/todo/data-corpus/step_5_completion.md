...
- [ ] Папка перемещена: `todo/data-corpus/` → `done/data-corpus/`  (после согласования мержа)
- [ ] **Цепочка влития веток (V3) согласована с пользователем ДО мержа.** Ветка
      `feature/data-corpus` отходит от `feature/realtime-bot-ingest` (НЕ от dev) —
      нарушение git-стратегии глобального CLAUDE.md. При влитии в dev потащит коммиты
      realtime-bot-ingest. Решить порядок: либо сначала влить родителя
      (realtime-bot-ingest → dev), затем data-corpus → dev; либо цепочкой. Как было в
      digest-scheduler (V4 того плана). Зафиксировать порядок с пользователем.
- [ ] **Готов вход для плана `prod-deploy`:** локальный корпус чистый и
      единообразный, дамп этой БД поедет на VPS (план B, шаг "корпус на прод")
