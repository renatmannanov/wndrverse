...
### Step 5 (smoke) пройден (2026-05-30)
- Группа `raymann_agents`. **Реальный chat_id = `-1003905781841`** (НЕ
  `-1003968221945` из `agents/_claude/.env` — то была другая группа). Узнали из
  лога бота (`skip: no topic for chat=...`), внесли в `topic_map.json`.
- Токен бота-ингестора лежит в `.env` как `BOT_TOKEN_INGEST` (юзер по ошибке
  положил в `BOT_TOKEN`, переименовали — отдельного токена куратора в локальном
  .env не было).
- Privacy mode у бота выключен, иначе обычные сообщения групп не приходят.
- Прогон: 4 фрагмента (msg 14,15,16,19), пропуски (17,18) = медиа-без-подписи →
  скип (ок). reply пишет `metadata.reply_to_msg_id`. Дедуп: повторный ingest того
  же external_id → `duplicates_skipped:1`, текст не перезаписан.
- Запуск бота в фоне: `python -m bot.ingest_bot`. Telegram держит апдейты ~24ч,
  но сообщения ДО добавления бота / до privacy-off не приходят (поймали на этом).

### Блок на step_5 (выполнено — оставлено для истории)
